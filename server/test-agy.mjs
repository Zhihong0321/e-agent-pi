...
export async function handleTestAgy(req, res, url) {
  ...
  if (req.method === "POST" && pathname === "/api/test-agy/auth/start") {
    ...
    // Clean up expired credentials to prompt fresh auth
    const { storageCredsPath, homeCredsPath } = getGeminiPaths();
    await unlink(storageCredsPath).catch(() => {});
    await unlink(homeCredsPath).catch(() => {});

    // Spawn agy to capture the official live OAuth URL generated by CLI
    const bin = resolveAgyBin();
    let authUrl = null;
    let authStderr = "";
    ...
    try {
      ...
      activeAuthSession = { process: child, url: null, startedAt: Date.now(), output: [] };
    ...
    jsonResponse(res, 200, {
      ...
      instructions: "1. Open the Google auth URL. 2. Sign in with Gemini Pro account. 3. Copy authorization code and submit.",
    });
    return;
  }
  ...
  if (req.method === "POST" && pathname === "/api/test-agy/auth/submit") {
    ...
    // If active agy process is waiting on stdin, pipe the code!
    if (activeAuthSession?.process && !activeAuthSession.process.killed) {
      ...
      jsonResponse(res, 200, {
        ok: true,
        message: "Code piped to agy process. Credentials persisted.",
        auth,
        output,
      });
      return;
    ...
    // Fallback: Direct exchange with Google OAuth token endpoint
    try {
      ...
      jsonResponse(res, 200, {
        ok: true,
        message: "Successfully exchanged authorization code and persisted credentials.",
        auth,
      });
    } catch (err) {
    ...
    <div id="tab-p2" class="tab-pane">
      <div class="card">
        ...
        <div id="oauth-flow" style="display: none; margin-bottom: 20px;">
          ...
          <div class="status-label">2. Paste authorization code returned by Google:</div>
          <input type="text" id="oauth-code" placeholder="4/0AWtg... authorization code">
